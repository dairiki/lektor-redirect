...
class TestRedirect(object):
    ...
    @pytest.mark.parametrize("record_path, url_path, redirect_path", [
...
class TestRedirectBuildProgram:
    ...
    def img_source(self, pad):
        return Redirect(pad.get("/images/apple-pie.jpg"),
                        "/images/apple-cake.jpg")
    ...
    def test_produce_non_html_artifacts(self, img_source, build_state, mocker):
        ...
        declare_artifact = mocker.patch.object(
            build_program, "declare_artifact"
        )
    ...
    def test_build_artifact(self, source, build_program, mocker):
        ...
        assert artifact.mock_calls == [
            mock.call.render_template_into(
                "redirect.html", this=source, values=values
            ),
        ]

    def test_build_artifact_records_dependency(self, source, build_program,
                                               env, context, mocker):
...
class TestRedirectMapBuildProgram:
    ...
    @pytest.mark.parametrize("project_url, prefix", [
        (None, "/"),
...
class TestRedirectIndex(object):
    @pytest.fixture
    def index(self, pad):
    ...
    @pytest.mark.parametrize("source_path, url_path, conflict_path", [
    ...
    def test_check_for_conflict(
        self, index, pad, source_path, url_path, conflict_path
    ):
        source = pad.get(source_path)
...
class TestRedirectPlugin(object):
    @pytest.fixture
    def plugin(self, env):
    ...
    def test_redirect_from_field_is_configurable(self, plugin,
                                                 open_config_file):
    ...
    @pytest.mark.parametrize("map_file, map_url", [
        (".redirect.map", "/.redirect.map"),
    ])
    def test_redirect_map_url(
        self, plugin, map_file, map_url, open_config_file
    ):
        with open_config_file() as inifile:
    ...
    def test_on_setup_env(self, env, plugin):
        ...
        assert env.virtual_sources["redirect-map"] \
            == RedirectMap._vpath_resolver
        assert plugin.resolve_url in env.custom_url_resolvers

    def test_on_setup_env_fails_if_alts_enabled(
        self, env, plugin, open_site_config
    ):
        with open_site_config() as inifile:
    ...
    @pytest.mark.parametrize("source_path, url_paths", [
    ...
    def test_generate_redirects_ignores_redirect_to_self(self, plugin, pad,
                                                         set_redirect_from):
        set_redirect_from("/about/more-detail", ["/about",
                                                 "/about/",
                                                 "about-this.html"])
        pad.cache.flush()
    ...
    def test_generate_redirects_skips_conflicts(self, plugin, pad,
                                                set_redirect_from,
                                                captured_reports):
    ...
    @pytest.mark.parametrize("source_path, url_path, redirect_path", [
        ("/about", ['info'],
         "/about/more-detail@redirect/about/info"),
        ("/", ['details'],
         "/about/more-detail@redirect/details"),
        ("/", ['about', 'projects.html'],
         "/projects@redirect/about/projects.html"),
        ])
    ...
    def test_resolve_url_redirect_map_fails(
        self, plugin, pad, open_config_file
    ):
        with open_config_file() as inifile:
            inifile["redirect.map_file"] = ".redirect.map"
...
class Test_iter_redirect_urls(object):
    @pytest.fixture
    def record(self, pad):
...
class Test_normalize_url_path(object):
    @pytest.fixture
    def record(self, pad):
        return pad.get('/about')

    @pytest.mark.parametrize("url_path, normalized", [
        ("/foo", "/foo/"),
        ("/foo/", "/foo/"),
        ("///foo//", "/foo/"),
        ("/foo.txt", "/foo.txt"),
...
class Test_walk_records(object):
    @pytest.fixture
